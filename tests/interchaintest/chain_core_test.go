...
func TestCoreSDKCommands(t *testing.T) {
	...
	sdk47Genesis := []cosmos.GenesisKV{
		...
		cosmos.NewGenesisKV("app_state.stakingmiddleware.params.blocks_per_epoch", 5),
...
func testDistribution(ctx context.Context, t *testing.T, chain *cosmos.CosmosChain, users []ibc.Wallet) {
	var err error
	node := chain.GetNode()
	err = testutil.WaitForBlocks(ctx, 10, node)
	...
	newWithdrawAddr := "pica1hj83l3auyqgy5qcp52l6sp2e67xwq9xxun76km"
...
func testGov(ctx context.Context, t *testing.T, chain *cosmos.CosmosChain, users []ibc.Wallet) {
	...
	govModule := "pica10d07y265gmmuvt4z0w9aw880jnsr700jp7sqj5"
...
func testStaking(ctx context.Context, t *testing.T, chain *cosmos.CosmosChain, users []ibc.Wallet) {
	...
	t.Run("misc", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.EqualValues(t, "stake", params.BondDenom)
...
func testVesting(ctx context.Context, t *testing.T, chain *cosmos.CosmosChain, admin ibc.Wallet) {
	...
	t.Run("normal vesting account", func(t *testing.T) {
		acc = "pica1w8arfu23uygwse72ym3krk2nhntlgdfv97pez9"

		err = node.VestingCreateAccount(ctx, admin.KeyName(), acc, "111stake", endTime)
		require.NoError(t, err)
	...
	t.Run("perm locked account", func(t *testing.T) {
		acc = "pica135e3r3l5333094zd37kw8s7htn7087pr8s9mj7"

		err = node.VestingCreatePermanentLockedAccount(ctx, admin.KeyName(), acc, "112stake")
		require.NoError(t, err)
	...
	t.Run("periodic account", func(t *testing.T) {
		acc = "pica1hkar47a0ysml3fhw2jgyrnrvwq9z8tk7ead5k9"
